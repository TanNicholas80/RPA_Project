...
<div class="container mx-auto mt-10 px-4 md:px-0">
    ...
    <div class="md:flex items-start">
            ...
            <div>
                ...
                <div class="mt-4 flex gap-4 overflow-x-auto">
                    @forelse ($produk->portofolio as $index => $foto)
                        <div class="relative">
                            <img 
                                src="https://drive.google.com/thumbnail?id={{ $foto->foto_portofolio }}&sz=w1000-h800" 
                            ...
                            <div class="overlay absolute inset-0 bg-white opacity-40 rounded-md pointer-events-none {{ $loop->first ? 'hidden' : '' }}"></div>
                        </div>
                    @empty
                        <img src="https://via.placeholder.com/150" alt="Foto Galeri Dummy" class="w-20 h-20 object-cover rounded-md shadow-md">
                    @endforelse
                </div>
        ...
        <div class="md:w-1/2 md:pl-10">
            <p class="font-bold text-gray-800 bg-[#DFBE91] py-2 px-4 rounded-full inline-block">{{ $produk->kategori->nama_kategori }}</p>
            ...
            <div class="mt-6">
                @foreach (['detail' => 'Detail', 'aturan' => 'Aturan Pemesanan', 'tambahan' => 'Tambahan'] as $id => $title)
                    <div class="mb-4 border border-[#000000] rounded-[16px] max-w-[500px]">
                        <button class="w-full text-left px-4 py-3 flex justify-between items-center focus:outline-none transition-all duration-300" onclick="toggleAccordion('{{ $id }}-{{ $produk->id }}')" aria-expanded="false">
                            <h3 class="text-base font-medium">{{ $title }}</h3>
                            <span id="arrow-{{ $id }}-{{ $produk->id }}" class="arrow-icon w-8 h-8 flex items-center justify-center bg-[#764C31] rounded-full transition-transform duration-300 ease-in-out">
